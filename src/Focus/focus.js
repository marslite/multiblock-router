...
function Focus() {
    ...
    const handleSubmitLink = async (event) => {
        ...
        try {
            const response = await fetch(
                "http://192.168.0.2:3000/api/focus-model/",
                {
            ...
            if (!response.ok) {
                throw new Error(
                    `Where you expecting somethong: ${response.status}`
                );
            }
        } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        if (
            /^\d{2}:\d{2}$/.test(textBox1Value) &&
            /^\d{2}:\d{2}$/.test(textBox2Value)
        ) {
            ...
            if (isValidMinutes(startTime[1]) && isValidMinutes(endTime[1])) {
                ...
                if (startMinutes < endMinutes) {
                    ...
                    console.log(
                        testTimeCount,
                        "Checking here the time control"
                    );
                    ...
                    setErrorMessage(
                        "Please make sure the starting time is earlier than the ending time"
                    );
                }
            } else {
                setErrorMessage(
                    "Please make sure minutes are in the format 00 or 30"
                );
            }
        } else {
            setErrorMessage(
                "Please make sure both time values are in the format HH:mm (e.g., 12:34)"
            );
        }
    };
